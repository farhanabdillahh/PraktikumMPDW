...
```{r}
data<- read_excel("C://Users//FARHAN ABDILLAH//Documents//1 KULIAH//SEMESTER 5//MPDW//dataset_mpdw.xlsx", sheet = 2)
View(data)
str(data)
dim(data)
```
...
###Split Data
```{r}
training<-data[1:292,2]
testing<-data[293:365,2]
...
```{r}
ts.plot(data1.ts, xlab="Time Period ", ylab="Sales", main= "SMA N=12 World CO2 Emmisions")
points(data1.ts)
...
```{r}
ts.plot(data1.ts, xlab="Time Period ", ylab="Sales", main= "SMA N=3 World CO2 Emmisions")
points(data1.ts)
...
```{r}
ts.plot(data1.ts, xlab="Time Period ", ylab="Sales", main= "DMA N=12 World CO2 Emmisions by Industry")
points(data1.ts)
...
akurasi_comparison <- data.frame(
  Model = c("SMA Lambda=0.2", "SMA Lambda=0.7", "SMA Optimum"),
