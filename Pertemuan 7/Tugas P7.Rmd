...
```{r}
plot.ts(data.ts, lty=1, xlab="waktu", ylab="Kurs", main="Plot Data Kurs")
```
...
Identifikasi model menggunakan plot EACF dilakukan dengan melihat ujung segitiga pada pola segitiga nol. Dalam hal ini model tentatif yang terbentuk adalah ARIMA(4,1,5), ARIMA(4,1,6), ARIMA(4,1,7), ARIMA(4,1,8)
...
```{r}
model3.da=Arima(train.diff, order=c(4,1,5),method="ML")
summary(model3.da) #AIC=640.52
lmtest::coeftest(model3.da) #ar2 tidak signifikan
```

#### ARIMA(4,1,6)

```{r}
model4.da=Arima(train.diff, order=c(4,1,6),method="ML")
summary(model4.da) #555.96 
lmtest::coeftest(model4.da) #ma6 tidak signifikan
```

#### ARIMA(4,1,7)

```{r}
model5.da=Arima(train.diff, order=c(4,1,7),method="ML")
summary(model5.da) #AIC=550.06 
lmtest::coeftest(model5.da) #ma7 tidak signifikan
```

#### ARIMA(4,1,8)

```{r}
model6.da=Arima(train.diff, order=c(4,1,8),method="ML")
summary(model6.da) #AIC=612.75
lmtest::coeftest(model6.da) #ma6 tidak signifikan
```
```{r}
...
Berdasarkan pendugaan parameter di atas, nilai AIC terkecil dimiliki oleh model ARIMA(4,1,7) dan parameter model ARIMA(4,1,7). Oleh karena itu, model itu diambil walau terdapat parameter yang tidak signifikan.
...
Berdasarkan plot kuantil-kuantil normal, secara eksplorasi ditunjukkan sisaan tidak menyebar normal ditandai dengan titik titik yang cenderung tidak mengikuti garis $45^{\circ}$. Kemudian dapat dilihat juga lebar pita sisaan yang cenderung tidak sama menandakan bahwa sisaan memiliki ragam yang heterogen. Plot ACF dan PACF sisaan ARIMA(4,1,7) juga tidak signifikan pada 20 lag awal yang menandakan saling bebas. Kondisi ini akan diuji lebih lanjut dengan uji formal.
...
Berdasarkan uji Ljung-Box tersebut, didapat *p-value* sebesar 0.9301 yang lebih besar dari taraf nyata 5% sehingga tak tolak $H_0$ dan menandakan bahwa sisaan saling bebas. Hal ini berbeda dengan eksplorasi.
...
Berdasarkan uji Ljung-Box terhadap sisaan kuadrat tersebut, didapat *p-value* sebesar 0.7082 yang lebih dari taraf nyata 5% sehingga tak tolak $H_0$ dan menandakan bahwa ragam sisaan homogen.
...
Berdasarkan uji-ttersebut, didapat *p-value* sebesar 0.6943 yang lebih besar dari taraf nyata 5% sehingga tak tolak $H_0$ dan menandakan bahwa nilai tengah sisaan sama dengan nol. Hal ini berbeda dengan eksplorasi.

### Overfitting
```{r}
model5a.da=Arima(train.diff, order=c(5,1,7),method="ML")
summary(model5a.da) #AIC=562.94 
lmtest::coeftest(model5a.da) #ar5 NAN

model5b.da=Arima(train.diff, order=c(4,1,8),method="ML")
summary(model5b.da) #AIC=612.75
lmtest::coeftest(model5b.da) #ma6 tidak signifikan
```
Model ARIMA(4,1,7) sebelumnya memiliki AIC=550.06  (lebih kecil). Oleh karena itu, model terbaiknya setelah overfitting adalah ARIMA(4,1,7).
...
```{r}
#---FORECAST---#
ramalan.da <- forecast::forecast(model5.da, h = 94) 
ramalan.da
data.ramalan.da <- ramalan.da$mean
plot(ramalan.da)
```

Berdasarkan hasil plot ramalan di atas, dapat dilihat bahwa ramalan ARIMA(4,1,7) cenderung stabil hingga akhir periode. Selanjutnya, dapat dicari nilai akurasi antara hasil ramalan dengan data uji sebagai berikut.
